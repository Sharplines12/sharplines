...
export function PerformanceTrendChart({
  ...
  const max = Math.max(...points.map((row) => Math.abs(row.units)), 1);
  ...
  return (
    <div className="panel p-6">
      ...
      <div className="mt-6 flex min-h-[220px] items-end gap-3">
        {points.length ? (
          points.map((row) => {
            const height = Math.max((Math.abs(row.units) / max) * 160, 14);
            const positive = row.units >= 0;

            return (
              <div key={`${title}-${row.label}`} className="flex flex-1 flex-col items-center gap-3">
                <p className={cn("text-xs font-medium", positive ? "text-neon" : "text-rose-200")}>
                ...
                <div className="flex h-[168px] items-end">
                  <div
                    className={cn(
                      ...
                      positive && accent === "neon" && "from-neon/25 to-neon/80",
                      positive && accent === "aqua" && "from-aqua/25 to-aqua/80",
          ...
          <div className="rounded-[22px] border border-dashed border-white/10 bg-white/[0.02] p-6 text-sm text-mist/60">
            Not enough settled bets yet to draw a trend line.
          </div>
        )}
      </div>
    </div>
