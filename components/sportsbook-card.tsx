    ...
    <div className="surface p-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <h3 className="mt-2 text-3xl uppercase text-slate-950">{sportsbook.name}</h3>
        </div>
        <BadgeDollarSign className="h-5 w-5 text-aqua" />
      </div>
      <p className="mt-4 text-sm font-medium text-slate-900">{sportsbook.featuredOfferText}</p>
      <p className="mt-3 text-sm leading-7 text-slate-600">{sportsbook.summary}</p>
      ...
      {expanded ? <p className="mt-5 text-xs uppercase tracking-[0.16em] text-slate-400">{sportsbook.disclaimer}</p> : null}
