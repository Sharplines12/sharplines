...
export function PremiumCtaBlock({ title, copy, compact = false }: PremiumCtaBlockProps) {
  return (
    <div className={`panel-strong ${compact ? "p-6" : "px-6 py-8 sm:px-10 sm:py-12"}`}>
      <PremiumBadge label="Premium membership" />
      <h2 className="mt-4 text-4xl uppercase text-white">{title}</h2>
      <p className="mt-4 max-w-3xl text-sm leading-7">{copy}</p>
